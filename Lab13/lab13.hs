...
-- addM mx my | mx == Nothing = Nothing
--            | my == Nothing = Nothing
--            | mx == Just x && my == Just y = Just (x + y)
-- addM mx my = fmap (+) mx <*> my

-- addM mx my = mx >>= (\a -> my >>= (\b -> return (a+b)))
...
-- ioNumber = (readLn::IO Float) >> \noin -> putStrLn( putStrLn $ "Intrare\n" ++ (show noin)) >>= \noout -> let noout = prelNo noin in 
ioNumberSecv = (readLn :: IO Float) >>= \noin ->
               putStrLn("Intrare\n" ++ show noin) >>= \n ->
               let noout = prelNo noin in
                  putStrLn "Iesire" >>= \_ -> print noout

ioNumberSecv2 = (readLn :: IO Float) >>= \noin ->
               putStrLn("Intrare\n" ++ show noin) >>
               let noout = prelNo noin in
                  putStrLn "Iesire" >> print noout
...
showPersonN person = "NUME: " ++ (name person)
showPersonA :: Person -> String
showPersonA person = "AGE: " ++ (age person)
